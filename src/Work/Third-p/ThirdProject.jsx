...
const ThirdProject = () => {
  ...
  const techStack = ["React", "Node.js", "MongoDB", "Tailwind CSS", "Cloudinary API", "ShadCn"]
        ...
        <div className="relative backdrop-blur-xl bg-white/10 border border-blue-600/30 rounded-2xl p-6 shadow-2xl hover:shadow-blue-500/20 transition-all duration-500 min-h-[650px] flex flex-col">
              ...
              {isHovered && (
                <>
                  <motion.button
                    ...
                    exit={{ opacity: 0, x: -10 }}
                    onClick={prevSlide}
                    className="absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/20 backdrop-blur-sm border border-blue-600/30 hover:bg-white/30 transition-all duration-200"
                  >
                  ...
                  <motion.button
                    ...
                    exit={{ opacity: 0, x: 10 }}
                    onClick={nextSlide}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/20 backdrop-blur-sm border border-blue-600/30 hover:bg-white/30 transition-all duration-200"
                  >
          ...
          >
            <p className="leading-relaxed mb-3">
              <strong>Cover AI</strong> is a fun SaaS-level image editing and generation tool powered by the Cloudinary API.It allows users to effortlessly enhance, transform, or generate new images with a seamless interface
            ...
            <div className="flex flex-wrap gap-2">
              {techStack.map((tech, index) => (
                <motion.span
                  ...
                  className="px-3 py-1.5 text-xs font-medium bg-white/10 backdrop-blur-sm border border-blue-600/30 rounded-full hover:bg-white/20 hover:border-blue-600/40 transition-all duration-200"
                >
                  {tech}
                </motion.span>
              ))}
          ...
          >
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white/10 backdrop-blur-sm border border-blue-600/30 rounded-xl hover:bg-white/20 hover:border-blue-600/40 transition-all duration-200 font-medium text-sm flex-1"
            >
            ...
            <motion.a
              ...
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500/20 to-purple-500/20 backdrop-blur-sm border border-blue-600/30 rounded-xl text-blue-600 dark:text-blue-400 hover:from-blue-500/30 hover:to-purple-500/30 hover:border-blue-600/40 transition-all duration-200 font-medium text-sm flex-1"
            >
